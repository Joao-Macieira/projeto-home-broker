...
func TestBuyAsset(t *testing.T) {
	...
	orderChanOutput := make(chan *Order)

	book := NewBook(orderChan, orderChanOutput, &wg)
	go book.Trade()
	...
	assert.Equal(0, order.PendingShares, "Order 1 should have 0 pending shares")

	assert.Equal("CLOSED", order2.Status, "Order 2 should be closed")
	assert.Equal(0, order2.PendingShares, "Order 2 should have 0 pending shares")

	assert.Equal(5, investorAssetPosition.Shares, "Investor 1 should havce 5 shares of asset 1")
	assert.Equal(5, investor2.GetAssetPosition("asset1").Shares, "Investor 5 should havce 5 shares of asset 1")
}
...
func TestBuyAssetWithDifferentAssets(t *testing.T) {
	...
	assert.Equal("OPEN", order.Status, "Order 1 should be opened")
	assert.Equal(5, order.PendingShares, "Order 1 should have 5 pendingShares")

	assert.Equal("OPEN", order2.Status, "Order 2 should be opened")
...
func TestBuyPartialAsset(t *testing.T) {
	...
	assert.Equal("OPEN", order2.Status, "Order 2 should still be open")
...
func TestBuyWithDifferentPrice(t *testing.T) {
	...
	assert.Equal("OPEN", order3.Status, "Order 3 should still be open")
...
func TestNoMatch(t *testing.T) {
	...
	assert.Equal("OPEN", order.Status, "Order 1 should still be open")
